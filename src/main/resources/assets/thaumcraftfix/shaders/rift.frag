...
// this is a monotonically increasing timer
// in the stock shader this is an int, but it gets partial ticks added in this renderer
uniform float time;

// these are the pitch and yaw rotation of the viewer
uniform float yaw;
uniform float pitch;

// this sampler has the starfield texture
// this is the way to texture the model - there are no normal texture coords
uniform sampler2D texture;

// this is the input position from the vertex shader
in vec4 modelViewPosition;

// everything below here is the same as the normal rift shader (other than a few minor variable changes)
...
void main() {
    ...
    vec4 dir = normalize(-modelViewPosition);
